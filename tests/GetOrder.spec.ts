...
test('get order with extremely large id and get 400 error', async ({ request }) => {
    const response = await request.get('https://backend.tallinn-learning.ee/test-orders/123456789012345678901234567890');
    console.log('Response status:', response.status());
...
test('send multiple requests without exceeding rate limits', async ({ request }) => {
    ...
    for (let r = 0; r < 10; r++) { // This test sends 100 consecutive requests to the specified URL,in other words we send requests in a loop to exceed the limit
...
test('GET should return 429 Too Many Requests after multiple requests', async ({ request }) => {
    ...
    for (let r = 0; r < MAX_REQUESTS; r++) { // This test sends 100 consecutive requests to the specified URL,in other words we send requests in a loop to exceed the limit
        const response = await request.get('https://backend.tallinn-learning.ee/test-orders/1');
